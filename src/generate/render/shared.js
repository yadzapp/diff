...
export function fileButtons(site, base, locations) {
  ...
  return `<span class="title-actions">${locations
    .map((l) => {
      ...
      return `<a class="icon-btn icon-btn-sm icon-btn-gray file-btn" href="${fileLineHref(site, base, l.path, l.line)}" data-tip="${tip}" aria-label="${esc(label)}"><i class="ic ic-code" aria-hidden="true"></i></a>`;
    })
    .join('')}</span>`;
}
