...
// nothing on the page. Skipped on the development server and on localhost /
// loopback (e.g. `npm run preview`), so local traffic never reaches GA or PostHog.
const LOCAL = '/^(localhost|127\\.0\\.0\\.1|::1)$/.test(location.hostname)';

const GA = ANALYTICS_ID
  ? `<script>if(!${LOCAL}){window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}window.gtag=gtag;gtag('js',new Date());gtag('config','${ANALYTICS_ID}');var s=document.createElement('script');s.async=1;s.src='https://www.googletagmanager.com/gtag/js?id=${ANALYTICS_ID}';document.head.appendChild(s)}</script>`
  : '';
...
<div class="side-set">
<div class="verpicker">
<button class="select-ghost ver-btn" id="verBtn" aria-haspopup="true" aria-expanded="false" aria-label="Change build"><span class="ver-label"></span><i class="ic ic-chev"></i></button>
...
<script>try{const b=location.pathname.match(/^\\/v\\/([^/]+)\\//)?.[1];const p=location.pathname.replace(/^\\/v\\/[^/]+\\//,'/').slice(1);const site=!p||/^(community|about|credits|release-notes|changelog|mod-check|guides|styleguide)(\\/|$)/.test(p);const n=b?sessionStorage.getItem('build-name:'+b):(site&&(sessionStorage.getItem('build-name:latest')||localStorage.getItem('build')));if(n)document.querySelector('.ver-label').textContent=n}catch(e){}</script>
