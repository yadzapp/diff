...
function rows(node, path) {
  ...
  const dirs = [...node.dirs.keys()].sort(byName).map((name) => {
    ...
    return `<li><details${open ? ' open' : ''}><summary><code>${esc(name)}</code> ` +
      `<span class="count text-sm font-normal text-fg2">${kid.count.toLocaleString('de-DE')}</span></summary>` +
...
function newColumn() {
  ...
  column.innerHTML = '<p class="filetree-title mt-0 mb-3 text-xs font-semibold uppercase tracking-wider text-fg2 leading-[calc(var(--text-2xl)*var(--text-2xl--line-height))]">All files</p><ul class="tree"></ul>';
  return column;
}
