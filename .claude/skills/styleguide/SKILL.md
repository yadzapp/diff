...
| Tag | `note-tag`, `note-tag-note`, `note-tag-warn`, `note-tag-exp`, `note-tag-removed`, size `note-tag-sm` | `site/app/tag.js` |
...
| Banner | `doc-note` / `doc-removed` + `stale-banner` + tag (`note` / `warn` / `exp` / `removed`) | `site/app/banner.js` |
